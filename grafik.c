// V 1.03 History: see comment at the end of this file
//
...
typedef struct
{
	...
	unsigned char CharHeight;	// How many pixel heigth (real pixel data, not including any empty pixel for frame)
...
bool LCD_Dialog(char *title, char *text, unsigned int BackColor, unsigned int ForeColor)  {
  uint8_t x,y,i,len;
  LCD_Cls(BackColor);
  ...
  {	//check for pressed Key
	switch(keyhit())  
	{
		case KEY_RIGHT_1:	return 1;
		case KEY_LEFT_1:	return 0;
	}
  }
}
...
//-------------------------------------------------------------------------------
//calculates 2 byte color from 1 byte color (332 RGB); used by Bitmap_256low... bitmap routines
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
static void Bitmap_65k_uncompressed(const prog_uint16_t Pbmp[], unsigned int PixelCount)
{
...
//-------------------------------------------------------------------------------
static void Bitmap_65k_compressed(const prog_uint16_t Pbmp[], unsigned int PixelCount)
{
...
//-------------------------------------------------------------------------------
static void Bitmap_256low_uncompressed(const prog_uint8_t Pbmp[], unsigned int PixelCount)
{
	for(unsigned int i=0; i<PixelCount; ++i)
	{
...
//-------------------------------------------------------------------------------
static void Bitmap_256low_compressed(const prog_uint8_t Pbmp[], unsigned int PixelCount)
{
...
//-------------------------------------------------------------------------------
static void Bitmap_256high_uncompressed(const prog_uint8_t Pbmp[], const prog_uint16_t ColorTable[], unsigned int PixelCount)
{
	for(unsigned int i=0; i<PixelCount; ++i)
	{
...
//-------------------------------------------------------------------------------
static void Bitmap_256high_compressed(const prog_uint8_t Pbmp[], const prog_uint16_t ColorTable[], unsigned int PixelCount)
{
...
//-------------------------------------------------------------------------------
void LCD_Bitmap_65k(int x1, int y1, int x2, int y2, const prog_uint16_t Pbmp[], bool Compressed)  //draw bitmap
{	
	LCD_Window(x1, y1, x2, y2);    // Define Window	
...
//-------------------------------------------------------------------------------
void LCD_Bitmap_256low(int x1, int y1, int x2, int y2, const prog_uint8_t Pbmp[], bool Compressed)
{	
	LCD_Window(x1, y1, x2, y2);    // Define Window	
...
//-------------------------------------------------------------------------------
void LCD_Bitmap_256high(int x1, int y1, int x2, int y2, const prog_uint8_t Pbmp[], const prog_uint16_t ColorTable[], bool Compressed)
{
	LCD_Window(x1, y1, x2, y2);    // Define Window	
