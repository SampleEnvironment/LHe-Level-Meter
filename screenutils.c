...
void draw_double(double number, uint8_t x, uint8_t y, uint8_t prec, char *unit, unsigned int color)
{
	...
	{	
		case 1:	//convert number to string (4 digits total with 1 decimal place)
					dtostrf(number,4,1,temp);
					break;
		case 2:	
					dtostrf(number,5,2,temp);
					break;
		case 3:	
					dtostrf(number,6,3,temp);
					break;
		case 4:	
					dtostrf(number,7,4,temp);
					break;
		default:	
					dtostrf(number,4,1,temp);
	}
...
void paint_he_level(double he_level)
{
	...
	{	
		...
		//uberleer
		case 0://-3:	
					LCD_Print("0%", 25, 37, 2, 4,4, ERR, BGC);
					break;
		default:	
					if(he_level < 100)
					{
...
void paint_main(uint16_t nr, char *position, double he_level, uint8_t batt, uint8_t critical_batt, bool update) 
{
	...
	if (!update)
	{
		...
		//HE-Level
		paint_he_level(he_level);
			
		paint_buttons("meas","opts",0);
...
void paint_offline(uint16_t nr, char *position, double he_level, uint8_t batt, uint8_t critical_batt, bool update)
{
	if(!update)
	{
		paint_main(nr,"OFF",he_level,batt,critical_batt,0);
		paint_buttons("meas","opts",0);
	}
	else {
		//HE-Level
...
void paint_opt_values_page5(double adc_span, double adc_zero)
{
...
void paint_current_opt_page3(uint8_t option, uint8_t key)
{
	if(KEY_LEFT_2 == key)
	{
		switch(option) 
		{
			case 1:	//1 is active, 4 was active
					LCD_Print(STRING_POS, 2, 20, 2, 1,1, ERR, BGC);
					LCD_Print(STRING_FILLING_TIMEOUT, 2, 80, 2, 1,1, FGC, BGC);
					break;
			case 2:	//2 is active, 1 was active
		...
		switch(option) 
		{
			...
			case 3:	//3 is active, 4 was active
					...
					LCD_Print(STRING_FILLING_TIMEOUT, 2, 80, 2, 1,1, FGC, BGC);
					break;
			case 4:	//4 is active, 1 was active
					LCD_Print(STRING_FILLING_TIMEOUT, 2, 80, 2, 1,1, ERR, BGC);
					LCD_Print(STRING_POS, 2, 20, 2, 1,1, FGC, BGC);
					break;
			default:
...
void paint_measure(uint16_t nr, char *position, double he_level, uint8_t batt, uint8_t critical_batt, bool update) 
{
	...
	if (!update)
	{
		LCD_Cls(BGC);
		//device id
		LCD_Print("Dev-Nr:", 1, 1, 2, 1,1, FGC, BGC);
		itoa(nr, temp, 10);
		LCD_Print(temp, 65, 2, 2, 1,1, ERR, BGC);
...
//updates postion letters/digits on filling login 
bool update_filling_pos(char (*pos_letters)[FILL_LETTERS_LENGTH], uint8_t (*pos_ranges)[FILL_RANGES_LENGTH], char *pos)
{
	char temp[50];
	
	//filling position login
	bool fill_pos_login = true;
